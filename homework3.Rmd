...
U_n.path= function(X, D){
  ...
  sigmas = t(sapply(A.h0s, function(a)sigma.estimator(X.tr, a)))
  # Computing the likelihoods of each pair (A.h0, sigma.h0)
  likelihoods = t(sapply(1:length(sigmas), function(idx)log_likelihood(X.tr, A.h0s[[idx]], sigmas[idx])))
  
  # Finding the idx that give us the maximum likelihood
  mle_idx = which(likelihoods==max(likelihoods))[1] # maximum likelihood over h0
