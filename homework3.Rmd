...
vectorized_log_likelihood = function(X, A, sigma.square)
  return(-sum(sapply(1:p, function(j) (compute.middle(j,p)/(2*sigma.square)+(n/2*log(sigma.square))))))
...
sigma.estimator = function(X, A){
  n = dim(X)[1]
  p = dim(X)[2]
  return(sum(sapply(1:p, function(j)compute.middle(j, p)))/(n*p))
}
...
test_link = function(m=1e3, alpha=.05, h0=T){
  ...
  A.link.hidden[2, 1] = 0#as.integer(!h0)
  A.link.hidden[6, 3] = 0
